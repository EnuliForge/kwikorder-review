...
const ALLOWED_NEXT: Record<Status, Status[]> = {
  ...
  ready:      ["cancelled"], // Bar doesn't deliver; runner does
  delivered:  [],
  cancelled:  [],
  completed:  [],
};
...
export default function BarKDS() {
  ...
  const load = async () => {
    try {
      setErr(null);
      const r = await fetch(`/api/kds/list?stream=drinks`, { cache: "no-store" });
  ...
  const startPreparing = async (t: Ticket) => {
    ...
    try {
      const res = await fetch("/api/tickets/update-status", {
        ...
        body: JSON.stringify({ order_code: t.order_code, stream: "drinks", next_status: "preparing" }),
      });
  ...
  const markReady = async (t: Ticket) => {
    ...
    try {
      const res = await fetch("/api/tickets/update-status", {
        method: "PATCH",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ order_code: t.order_code, stream: "drinks", next_status: "ready" }),
      });
      const j = await res.json();
      if (!res.ok || !j.ok) throw new Error(j.error || "Update failed");
      await load();
  ...
  // Optional: sort by status then created_at
  ...
  return (
    <main className="mx-auto max-w-6xl p-4">
      <header className="mb-4 flex items-center justify-between">
        ...
        <div className="text-sm text-gray-600">Stream: <span className="font-semibold">Drinks</span></div>
      </header>
